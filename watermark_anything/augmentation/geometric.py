...
class Perspective(nn.Module):
    ...
    def forward(self, image, mask, distortion_scale=None):
        ...
        # Parse string modes (e.g. "0.1_val")
        if isinstance(distortion_scale, str):
            if distortion_scale.endswith("_val"):
                mode_flag = "val"
                distortion_scale = float(distortion_scale.replace("_val", ""))
            else:
        ...
        if mode_flag == "val":
            startpoints, endpoints = self.get_perspective_params_val(width, height, distortion_scale)
        else:
    ...
    @staticmethod
    def get_perspective_params_val(width, height, distortion_scale, ratios=None):
        half_height = height // 2
        half_width = width // 2
        dx = distortion_scale * half_width
        dy = distortion_scale * half_height
        # Default asymmetric corner ratios (override with ``ratios``)
        if ratios is None:
            ratios = {
                "topleft":  (0.1, 1.0),
                "topright": (0.8, 0.4),
        ...
        # Build four corner endpoints from ratios
        topleft = [
            int(ratios["topleft"][0] * dx),
            int(ratios["topleft"][1] * dy)
        ]
        topright = [
            int(width - 1 - ratios["topright"][0] * dx),
            int(ratios["topright"][1] * dy)
        ]
        botright = [
            int(width - 1 - ratios["botright"][0] * dx),
            int(height - 1 - ratios["botright"][1] * dy)
        ]
        botleft = [
            int(ratios["botleft"][0] * dx),
            int(height - 1 - ratios["botleft"][1] * dy)
        ]
        startpoints = [
            [0, 0],
            [width - 1, 0],
...
if __name__ == "__main__":
    ...
    transformations = [
        ...
        (Perspective, ["0.1_val", "0.3_val"]),       # distortion_scale
        # (HorizontalFlip, [])             # No parameters needed for flip
    ]

    # Load images
    imgs = [
        Image.open("assets/images/gauguin_256.jpg")
    ]
