...
def get_masked_bit_mode(mask_preds, post_info_bits, active_mask):
    ...
    Returns:
        Tensor: [B, 32]  per-batch mode of 32-bit vectors over valid pixels
    """
    # Step 1: combine masks -> valid region [B, H, W]
    combined_mask = mask_preds.int() & active_mask.int()
    # save_image(combined_mask.float(), 'output/ldpc/combined_mask.png')
    # Step 2: gather valid 32-bit vectors per batch
    B, C, H, W = post_info_bits.shape
    mode_bits = []

    for b in range(B):
        # 2.1 flatten spatial dims, keep valid positions [H*W, 32]
        bits_batch = post_info_bits[b].permute(1, 2, 0).reshape(-1, C)  # [H*W, 32]
        valid_mask = combined_mask[b].flatten().bool()  # [H*W]
        valid_bits = bits_batch[valid_mask]  # [N, 32], N = number of valid pixels

        # 2.2 empty valid region -> zeros
        if len(valid_bits) == 0:
            mode_bit = torch.zeros(C, dtype=torch.int32, device=post_info_bits.device)
            mode_bits.append(mode_bit)
            continue

        # 2.3 mode over binary vectors
        # Hash each 32-bit pattern to int64, then take mode of ints
        weights = (1 << torch.arange(C, device=valid_bits.device, dtype=torch.int64)).unsqueeze(0)  # [1, 32]

        int_values = (valid_bits.to(torch.int64) * weights).sum(dim=1)  # [N]
        ...
        mode_bit = ((mode_value.unsqueeze(0) & weights) > 0).to(torch.int32).squeeze(0)  # [32]
        mode_bits.append(mode_bit)

    mode_bits = torch.stack(mode_bits)  # [B, 32]
    return mode_bits
